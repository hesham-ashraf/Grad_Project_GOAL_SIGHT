...
class SpeedAndDistance_Estimator():
    ...
    def __init__(self, frame_rate=24, min_frames_for_player=30, max_players_for_report=22):
        self.frame_rate = frame_rate
        self.dt = 1.0 / frame_rate                                  
        self.min_frames_for_player = min_frames_for_player
        self.max_players_for_report = max_players_for_report
    ...
    def _get_identity_key(self, track_id, track_info):
        """Use canonical identity when present, otherwise preserve legacy track_id behavior."""
        canonical_id = track_info.get('canonical_id') if track_info else None
        if canonical_id:
            return canonical_id
    ...
    def add_speed_and_distance_to_tracks(self, tracks):
        ...
        5. Accumulate effective distance from final speed (excluding dead ball frames)
        """
        ...
        for object_type, object_tracks in tracks.items():
            ...
            for frame_num in range(number_of_frames):
                ...
                for track_id, track_info in object_tracks[frame_num].items():
                    identity_key = self._get_identity_key(track_id, track_info)

                    if identity_key in seen_identity_keys:
                        # Conflicting duplicate canonical assignments are handled later,
                        # but we skip double-counting here so analytics stay bounded.
                        tracks[object_type][frame_num][track_id]['speed'] = track_info.get('speed', 0.0)
                        tracks[object_type][frame_num][track_id]['distance'] = self.total_distance.get(identity_key, 0.0)
                        tracks[object_type][frame_num][track_id]['avg_speed'] = self.average_speed.get(identity_key, 0.0)
                        tracks[object_type][frame_num][track_id]['identity_conflict'] = True
                        continue

                    seen_identity_keys.add(identity_key)
                    tracks[object_type][frame_num][track_id]['identity_conflict'] = False

                    if identity_key not in self.frame_count:
                        self.frame_count[identity_key] = 0
                    self.frame_count[identity_key] += 1
                    ...
                    if identity_key not in self.previous_positions:
                        self.previous_positions[identity_key] = current_pos
                        self.total_distance[identity_key] = 0.0
                        tracks[object_type][frame_num][track_id]['speed'] = 0.0
                        tracks[object_type][frame_num][track_id]['distance'] = 0.0
                        continue
                    
                                           
                    prev_pos = self.previous_positions[identity_key]
                    ...
                    if frame_distance < self.min_movement:
                        # Always refresh previous position on sub-threshold motion to avoid
                        # stale anchors that can inflate the next measured displacement.
                        self.previous_positions[identity_key] = current_pos

                                                                                       
                        if identity_key in self.previous_speeds:
                                                   
                            final_speed_mps = self.previous_speeds[identity_key] * 0.8
                            self.previous_speeds[identity_key] = final_speed_mps
                        else:
                            final_speed_mps = 0.0

                        speed_kmh = final_speed_mps * 3.6

                        # Keep distance consistent with the displayed/filtered speed value.
                        # We do not add raw frame_distance directly because speed has already
                        # been corrected by smoothing and physical constraints.
                        if not is_excluded:
                            effective_distance = final_speed_mps * self.dt
                            self.total_distance[identity_key] += effective_distance

                        if identity_key not in self.speed_values:
                            self.speed_values[identity_key] = []
                        self.speed_values[identity_key].append(speed_kmh)
                    ...
                    constrained_speed = self._apply_constraints(identity_key, raw_speed_mps)
                    
                                                                               
                                                   
                    savgol_smoothed = self._apply_savgol_smoothing(identity_key, constrained_speed)
                    
                                                                              
                    final_speed_mps = self._apply_ema_smoothing(identity_key, savgol_smoothed)
                    
                                     
                    speed_kmh = final_speed_mps * 3.6
                    
                                                                                   
                    # Distance is integrated from final_speed_mps so reported distance and
                    # reported speed remain mathematically aligned frame-by-frame.
                    if not is_excluded:
                        effective_distance = final_speed_mps * self.dt
                        self.total_distance[identity_key] += effective_distance
                    ...
                    tracks[object_type][frame_num][track_id]['speed'] = speed_kmh
                    tracks[object_type][frame_num][track_id]['distance'] = self.total_distance[identity_key]
                    
                                              
                    self.previous_positions[identity_key] = current_pos
        
                                                 
        # Derive average speed from total distance over appearance time so summary
        # metrics remain physically consistent: avg_speed ~= distance / time.
        for identity_key in self.total_distance.keys():
            if self.frame_rate > 0:
                appearance_time_s = self.frame_count.get(identity_key, 0) / self.frame_rate
            else:
                appearance_time_s = 0.0

            if appearance_time_s > 0:
                self.average_speed[identity_key] = (self.total_distance[identity_key] / appearance_time_s) * 3.6
            else:
                self.average_speed[identity_key] = 0.0
        
                                                            
        total_players_before = len(self.average_speed)
        valid_player_ids = set()
        
        for identity_key in list(self.average_speed.keys()):
            frames_appeared = self.frame_count.get(identity_key, 0)
            if frames_appeared >= self.min_frames_for_player:
                valid_player_ids.add(identity_key)
            else:
                                       
                del self.average_speed[identity_key]
                if identity_key in self.total_distance:
                    del self.total_distance[identity_key]
                if identity_key in self.speed_values:
                    del self.speed_values[identity_key]
        
        total_players_after = len(self.average_speed)

        dropped_by_cap = 0
        if self.max_players_for_report is not None and total_players_after > self.max_players_for_report:
            ranked_identities = sorted(
                valid_player_ids,
                key=lambda identity: (
                    self.frame_count.get(identity, 0),
                    self.total_distance.get(identity, 0.0),
                ),
                reverse=True,
            )
            kept_identities = set(ranked_identities[: self.max_players_for_report])
            dropped_identities = set(valid_player_ids) - kept_identities
            dropped_by_cap = len(dropped_identities)

            for identity_key in dropped_identities:
                self.average_speed.pop(identity_key, None)
                self.total_distance.pop(identity_key, None)
                self.speed_values.pop(identity_key, None)

            valid_player_ids = kept_identities
            total_players_after = len(self.average_speed)
        ...
        if dropped_by_cap > 0:
            print(
                f"[Player Filter] Dropped {dropped_by_cap} low-persistence identities "
                f"to match max_players_for_report={self.max_players_for_report}"
            )
        ...
        for object_type, object_tracks in tracks.items():
            ...
            for frame_num in range(len(object_tracks)):
                for track_id, track_info in object_tracks[frame_num].items():
                    identity_key = self._get_identity_key(track_id, track_info)
                    if identity_key in valid_player_ids and identity_key in self.average_speed:
                        tracks[object_type][frame_num][track_id]['avg_speed'] = self.average_speed[identity_key]
        
                          
        if self.average_speed:
            print(f"[Opta Speed/Distance] Players tracked: {len(self.average_speed)}")
            sample_consistency = []
            for identity_key in list(self.average_speed.keys())[:5]:
                appearance_time_s = (self.frame_count.get(identity_key, 0) / self.frame_rate) if self.frame_rate > 0 else 0.0
                sample_consistency.append(
                    (
                        identity_key,
                        self.total_distance.get(identity_key, 0.0),
                        appearance_time_s,
                        self.average_speed.get(identity_key, 0.0),
                    )
                )
            print(
                "[Opta Speed/Distance] Sample consistency (id, dist_m, time_s, avg_km_h): "
                f"{[(pid, f'{dist:.1f}', f'{t:.1f}', f'{avg:.1f}') for pid, dist, t, avg in sample_consistency]}"
            )
    ...
    def save_player_insights(self, tracks, output_path="videos_insights.txt"):
        ...
        player_data = []
        for identity_key in self.average_speed.keys():
            avg_speed = self.average_speed.get(identity_key, 0.0)
            distance = self.total_distance.get(identity_key, 0.0)
            player_data.append({
                'player_id': identity_key,
                'avg_speed_kmh': avg_speed,
                'total_distance_m': distance
            })
        
                           
        player_data.sort(key=lambda x: str(x['player_id']))
    ...
    def calculate_line_height(self, tracks):
        ...
        for frame_num, player_track in enumerate(tracks.get('players', [])):
            for player_id, track_info in player_track.items():
                ...
                identity_key = self._get_identity_key(player_id, track_info)
                
                if pos is not None and team is not None:
                    if identity_key not in player_avg_positions:
                        player_avg_positions[identity_key] = {'x_values': [], 'team': team}
                        player_teams[identity_key] = team
                    
                    player_avg_positions[identity_key]['x_values'].append(float(pos[0]))
    ...
    def calculate_convex_hull_area(self, tracks):
        ...
        for frame_num, player_track in enumerate(tracks.get('players', [])):
            ...
            for player_id, track_info in player_track.items():
                ...
                if pos is not None and team is not None and identity_key not in seen_identity_keys:
                    seen_identity_keys.add(identity_key)
                    if team == 1:
    ...
    def calculate_pressing_index(self, tracks, pressing_radius=7.0, closing_speed_threshold=2.0):
        ...
        for frame_num, player_track in enumerate(tracks.get('players', [])):
            ...
            for player_id, track_info in player_track.items():
                ...
                if pos is None or team is None or identity_key in seen_identity_keys:
                    continue
                seen_identity_keys.add(identity_key)
                ...
                closing_speed = 0.0
                if frame_num > 0:
                    prev_key = (frame_num - 1, identity_key)
                ...
                prev_distances_to_ball[(frame_num, identity_key)] = distance_to_ball

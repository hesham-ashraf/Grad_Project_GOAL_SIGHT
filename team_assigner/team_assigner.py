...
class TeamAssigner:
    ...
    def _get_identity_key(self, player_id, track_info=None):
        """Prefer canonical identity when available, otherwise use transient tracker id."""
    ...
    def get_player_team(self, frame, player_bbox, player_id, track_info=None, confirm_threshold=8):
        """
        ...
        if identity_key in self.player_team_dict:
            return self.player_team_dict[identity_key]
        ...
        if player_color is None:
            ...
            if identity_key in self.player_color_history and self.player_color_history[identity_key]:
                return self.player_color_history[identity_key][-1]
            return None
        ...
        if identity_key not in self.player_color_history:
            self.player_color_history[identity_key] = []
        
        self.player_color_history[identity_key].append(team_id)
        
                                                   
        if len(self.player_color_history[identity_key]) > 30:
            self.player_color_history[identity_key] = self.player_color_history[identity_key][-30:]
        
                                                            
        if len(self.player_color_history[identity_key]) >= confirm_threshold:
            team_counts = {1: 0, 2: 0}
            for t in self.player_color_history[identity_key]:
                if t in team_counts:
                    team_counts[t] += 1
            ...
            if confidence >= 0.55:
                self.player_team_dict[identity_key] = final_team                    
                return final_team
        
                                                         
        if self.player_color_history[identity_key]:
            team_counts = {1: 0, 2: 0}
            for t in self.player_color_history[identity_key]:
                if t in team_counts:
                    team_counts[t] += 1

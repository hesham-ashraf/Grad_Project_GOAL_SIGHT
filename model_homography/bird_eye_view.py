...
class BirdEyeView:
    ...
    def __init__(
        ...
        self.player_trails: Dict[Any, List[Tuple[int, int]]] = {}
        self.ball_trail: List[Tuple[int, int]] = []
        
                                            
        self.previous_positions: Dict[Any, Tuple[float, float]] = {}
    ...
    def _get_identity_key(self, player_id: Any, player_info: Optional[Dict] = None):
        canonical_id = player_info.get("canonical_id") if player_info else None
        return canonical_id if canonical_id else player_id

    def _get_display_label(self, player_id: Any, player_info: Optional[Dict] = None) -> str:
        if player_info:
            jersey_number = player_info.get("jersey_number")
            if jersey_number not in (None, ""):
                return str(jersey_number)

            canonical_id = player_info.get("canonical_id")
            if canonical_id:
                return str(canonical_id).replace("player_", "P")

        if isinstance(player_id, np.integer):
            return str(int(player_id))
        return str(player_id)

    def _find_player_info_by_identity(self, frame_players: Dict, identity_key: Any) -> Dict:
        for raw_player_id, player_info in frame_players.items():
            if self._get_identity_key(raw_player_id, player_info) == identity_key:
                return player_info
        return {}
    ...
    def _update_trail(self, trail_dict: Dict, player_id: Any, position: Tuple[int, int]):
        """تحديث مسار اللاعب"""
    ...
    def draw(self, frame: np.ndarray, tracks: Dict, frame_idx: int,
        ...
        if self.show_trails:
            for player_id, trail in self.player_trails.items():
                ...
                if "players" in tracks and frame_idx < len(tracks["players"]):
                    player_info = self._find_player_info_by_identity(tracks["players"][frame_idx], player_id)
        ...
        if "players" in tracks and frame_idx < len(tracks["players"]):
            ...
            for player_id, info in tracks["players"][frame_idx].items():
                ...
                identity_key = self._get_identity_key(player_id, info)
                if identity_key in seen_identity_keys:
                    continue
                seen_identity_keys.add(identity_key)
                display_label = self._get_display_label(player_id, info)
                
                player_data.append((identity_key, display_label, map_x, map_y, x, y, team, has_ball))
        ...
        for player_id, display_label, map_x, map_y, x, y, team, has_ball in player_data:
            ...
            self._draw_player(minimap, map_x, map_y, team, has_ball, 
                             display_label if self.show_player_ids else None)

...
class Tracker:
    ...
    def _get_player_label(self, track_id, player_info=None):
        if player_info:
            jersey_number = player_info.get("jersey_number")
            if jersey_number not in (None, ""):
                return str(jersey_number)

            display_name = player_info.get("display_name") or player_info.get("name")
            if display_name:
        ...
        return str(int(track_id)) if isinstance(track_id, np.integer) else str(track_id)

    def _get_referee_label(self, referee_info=None):
        if not referee_info:
            return "REF"

        display_name = referee_info.get("display_name") or referee_info.get("name")
        if display_name:
            return str(display_name)

        canonical_id = referee_info.get("canonical_id")
        if canonical_id:
            return str(canonical_id).replace("ref_", "R")
    ...
    def draw_ellipse(self, frame, bbox, color, label=None):
        ...
        if label is not None:
            label = str(label)
            rectangle_width = max(rectangle_width, 18 + (len(label) * 10))
            x1_rect = x_center - rectangle_width//2
            x2_rect = x_center + rectangle_width//2
            cv2.rectangle(frame,
    ...
    def draw_annotations(self, video_frames, tracks, team_ball_control):
        ...
        for frame_num, frame in enumerate(video_frames):
            ...
            for track_id, player in player_dict.items():
                ...
                label = self._get_player_label(track_id, player)
                frame = self.draw_ellipse(frame, player_bbox, color, label)
            ...
            for _, referee in referee_dict.items():
                ...
                if is_valid_bbox(referee_bbox):
                    referee_label = self._get_referee_label(referee)
                    frame = self.draw_ellipse(frame, referee_bbox, (0, 255, 255), referee_label)

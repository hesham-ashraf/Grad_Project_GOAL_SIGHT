...
class PlayerBallAssigner():
    def __init__(self):
        ...
        self.last_assigned_identity = None

    def _get_identity_key(self, player_id, player_info):
    ...
    def assign_ball_to_player(self, players, ball_bbox):
        ...
        if assigned_player == -1 and self.last_assigned_identity is not None:
            best_identity_match = (-1, float('inf'))
            for player_id, player in players.items():
                if self._get_identity_key(player_id, player) != self.last_assigned_identity:
                    continue
                ...
                foot_center = ((player_bbox[0] + player_bbox[2]) / 2, player_bbox[3])
                dist_to_ball = measure_distance(foot_center, ball_position)
                if dist_to_ball < best_identity_match[1]:
                    best_identity_match = (player_id, dist_to_ball)

            if best_identity_match[0] != -1 and best_identity_match[1] < self.max_player_ball_distance * 1.5:
                assigned_player = best_identity_match[0]
        
        if assigned_player != -1:
            self.last_assigned_player = assigned_player
            self.last_assigned_identity = self._get_identity_key(assigned_player, players.get(assigned_player, {}))
